...
contract SlingshotFactory is BuildingKind {
    function use(Game ds, bytes24 buildingInstance, bytes24 mobileUnitID, bytes memory /*payload*/ ) public override {
        ...
        require(unitTeam == tileTeam, "SlingshotFactory: Unit does not belong to factory's team");
        ...
        override
    {
        ...
        require(team != Team.NONE, "SlingshotFactory: Tile does not belong to a team");

        TeamState memory teamState =
            LibTeamState.decodeTeamState(uint256(state.getData(zoneID, LibUtils.getTeamStateKey(team))));

        require(teamState.hasSlingshot, "SlingshotFactory: Team does not have slingshot blueprint");
    }
}
