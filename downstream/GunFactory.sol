...
contract GunFactory is BuildingKind {
    function use(Game ds, bytes24 buildingInstance, bytes24 mobileUnitID, bytes memory /*payload*/ ) public override {
        ...
        require(unitTeam == tileTeam, "GunFactory: Unit does not belong to factory's team");
        ...
        override
    {
        ...
        require(team != Team.NONE, "GunFactory: Tile does not belong to a team");

        TeamState memory teamState =
            LibTeamState.decodeTeamState(uint256(state.getData(zoneID, LibUtils.getTeamStateKey(team))));

        require(teamState.hasGun, "GunFactory: Team does not have longbow blueprint");
    }
}
